...
def get_db_connection():
    ...
    if ca_b64 and cert_b64 and key_b64:
        ...
        try:
            ca_f.write(base64.b64decode(ca_b64))
            ca_f.flush()
            cert_f.write(base64.b64decode(cert_b64))
            cert_f.flush()
            key_f.write(base64.b64decode(key_b64))
            key_f.flush()
...
async def get_history(usuario: Optional[str] = None):
    ...
    try:
        ...
        if usuario:
            cur.execute(
                "SELECT id, equipamento, severidade, laudo_tecnico, usuario, json_completo FROM historico_manutencao WHERE usuario = %s ORDER BY id DESC",
                (usuario,),
            )
        else:
            cur.execute(
                "SELECT id, equipamento, severidade, laudo_tecnico, usuario, json_completo FROM historico_manutencao ORDER BY id DESC LIMIT 50"
            )
        rows = cur.fetchall()
        cur.close()
...
async def diagnose(req: DiagnoseRequest):
    ...
    try:
        ...
        if not raw:
            raise Exception("IA não retornou dados.")
